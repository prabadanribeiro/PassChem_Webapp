...
function LessonButtons({ lesson, topicTitle, unitTitle }) {
    ...
    useEffect(() => {
        if (!lessonsLoading && !topicsLoading) { 
            ...
            if (currentIndex > 0 && (!topic || lessons[currentIndex - 1].topic === topic.id)) {
                setPreviousLesson(lessons[currentIndex - 1]);
            } else {
                setPreviousLesson(null);
            }

            if (currentIndex !== -1 && currentIndex < lessons.length - 1 && (!topic || lessons[currentIndex + 1].topic === topic.id)) {
    ...
    const generateLessonLink = (lesson) => {
        if (topicTitle) {
            return `/curriculum/${URLService.slugify(unitTitle)}/${URLService.slugify(topicTitle)}/${URLService.slugify(lesson.title)}`;
        }
        return `/curriculum/${URLService.slugify(unitTitle)}/${URLService.slugify(lesson.title)}`;
    };

    return (
        <div className='lesson-buttons-container'>
            {nextLesson && (
                <Link className='lesson-links' id='next-button' to={generateLessonLink(nextLesson)}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="m321-80-71-71 329-329-329-329 71-71 400 400L321-80Z"/></svg>
                </Link>
            )}
            {previousLesson && (
                <Link className='lesson-links' id='previous-button' to={generateLessonLink(previousLesson)}>
